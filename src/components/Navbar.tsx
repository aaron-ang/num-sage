    ...
    <nav className="bg-transparent pt-3">
      <div className="mx-auto flex max-w-screen-xl flex-wrap items-center justify-between px-8">
        ...
        <div className="hidden w-full md:block md:w-auto" id="navbar-default">
          <ul className="mt-2 flex flex-col rounded-lg font-medium md:mt-0 md:flex-row">
            {Object.entries(options).map(([key, value]) => (
              <li key={key}>
                <a
                  href={value}
                  className="block rounded px-3 py-2 text-2xl text-sub-color hover:bg-sub-color hover:text-accent md:text-3xl"
                >
                  {key}
                </a>
              </li>
